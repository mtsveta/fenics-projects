from dolfin.cpp.la import as_backend_type
...
def calculate_majorant_bar_mu_opt(u, y, beta, f_bar, A, invA, min_eig_A, lmbd, mesh, dim, C_FD):
    ...
    w_opt = (C_FD ** 2) * (1 + beta) / (beta * min_eig_A + (C_FD ** 2) * (1 + beta) * lmbd)
    ...
    var_m_f_one_minus_mu_opt = ((1 - mu_opt) * (1 - mu_opt)) * abs(inner(r_f, r_f))
...
def majorant_nd(v, y, H_div, f, A, invA, min_eig_A, eps, a, lmbd,
                error,
                mesh, dim, C_FD, C_Ntr,
                problem_params):
    """
    ...
    #f_bar = f
    maj, m_d, m_f, beta, var_m_d, var_m_f_w_opt = calculate_majorant_bar_mu_opt(v, y, beta, f_bar, A, invA, min_eig_A, lmbd, mesh, dim, C_FD)
...
def minorant(v_ref, v, mesh, ds, Vh, uN, uD_boundary, f, A, lmbd, conv, dim, test_params):
