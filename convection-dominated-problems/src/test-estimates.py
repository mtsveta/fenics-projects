...
#parameters["refinement_algorithm"] = "plaza_wifth_different_faces"
...
class TestEstimates():
    ...
    def __init__(self, test_num,
                       u_expr, grad_u_expr, f_expr, A_expr, lambda_1, lambda_expr, a_expr, eps,
                       uD_expr, uN_expr,
                       domain, dim):
    ...
    def update_problem_data(self, problem_params, eps):
        ...
        if dim == 1:
            ...
            A_values = [[eps, 0], [0,eps]]
            A = as_matrix(A_values)
        ...
        invA = inverse(A, A_values, self.dim)
        adjA = det(A) * invA
        min_eig_A = eps

        if problem_params['solution_tag'] == "predefined-solution":
            u_e = Expression(self.u_expr, eps=eps, degree=expression_degree)
            if dim == 1:
                grad_u_e = Expression(grad_u_expr[0], eps=eps, degree=expression_degree)
            elif dim == 2:
                grad_u_e = Expression((grad_u_expr[0], grad_u_expr[1]), eps=eps, degree=expression_degree)
            elif dim == 3:
                grad_u_e = Expression((grad_u_expr[0], grad_u_expr[1], grad_u_expr[2]), eps=eps,
                                      degree=expression_degree)

            grad_u_e = Expression((self.grad_u_expr[0], self.grad_u_expr[1]), eps=eps, degree=expression_degree)

        uD = Expression(self.uD_expr,eps=eps, degree=expression_degree)

        # Construct the expression for the right-hand side
        f = Expression(self.f_expr, eps=eps, degree=expression_degree)

        return A, invA, adjA, u_e, grad_u_e, uD, f, min_eig_A

    # Convert given data expressions form string format to the UFL Expressions
    def convert_problem_data_to_expressions(self, mesh, problem_params, eps_init):
        """
        ...
        # Define diffusion matrix A if it is constant over the whole domain
            ...
            if dim == 1:
                ...
                A_values = [[eps_init, 0], [0, eps_init]]
                A = as_matrix(A_values)

            elif dim == 3:
                A_values = [[eps_init, 0, 0],
                            [0, eps_init, 0],
                            [0, 0, eps_init]]
                A = as_matrix(A_values)
        ...
        if problem_params['solution_tag'] == "reference-solution":
            ...
            u_e = Expression(self.u_expr, eps=eps_init, degree=expression_degree)
            if dim == 1:
                grad_u_e = Expression(grad_u_expr[0], eps=eps_init, degree=expression_degree)
            elif dim == 2:
                grad_u_e = Expression((grad_u_expr[0], grad_u_expr[1]), eps=eps_init, degree=expression_degree)
            elif dim == 3:
                grad_u_e = Expression((grad_u_expr[0], grad_u_expr[1], grad_u_expr[2]), eps=eps_init, degree=expression_degree)
        ...
        return f, A, invA, adjA, min_eig_A, lmbd, a, uD, uN, u_e, grad_u_e
    ...
    def solve_problem_nd_t(self, mesh, boundary_facets, ds, dS,
        ...
        while h_min > eps_curr: # (i <= ref_num and maj > test_params['accuracy_level']) or
            ...
            u, delta = problem.solve_convection_reaction_diffusion(V, f, A, min_eig_A, lmbd,
            ...
            if test_params['solution_tag'] == "reference-solution":
                ...
                u_e, delta = problem.solve_convection_reaction_diffusion(V_exact, f, A, min_eig_A, lmbd,
            ...
            if test_params["error_estimates"] == True:
                ...
                # Calculate minorant
                min, phi = estimates.minorant(u_e, u, mesh, ds, V_exact, uN, self.boundary, f, A, lmbd, a, dim, test_params)
...
if __name__ == '__main__':
    ...
    # ---------------------------------------------------------------------------------------------------------------------#
    tests = {2: tests.quadratic_polynomial_solution_1d_sigma_1000, # checked
             ...
             56: tests.classic_example_1d_stynes_example_1_2}
    # ---------------------------------------------------------------------------------------------------------------------#
    ...
    test_params = {'refinement_tag': adaptive_tag,
                   ...
                   'STABILIZE': True,
                   'LOG_TO_FILE': False,
                   'expression_degree': expression_degree} # option that controls whether to log results into the file (True)
                                         # or to console (False)
    ...
    test = TestEstimates(test_num,
                         u_expr, grad_u_expr, f_expr, A_expr, min_eig_A, lambda_expr, a_expr, eps,
                         uD_expr, uN_expr,
                         domain, dim)
